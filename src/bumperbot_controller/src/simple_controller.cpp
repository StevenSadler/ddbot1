...
SimpleController::SimpleController(const std::string & name)
    ...
    , theta_(0.0)
{
    declare_parameter("wheel_radius", 0.038);
    declare_parameter("wheel_separation", 0.278);
...
void SimpleController::jointCallback(const sensor_msgs::msg::JointState & msg)
{
    ...
    if (dt.seconds() > 0)
    {
        phi_left = dp_left / dt.seconds();
        phi_right = dp_right / dt.seconds();
    }
    else
    {
        phi_left = 0.0;
    ...
    double linear = wheel_radius_ / 2 * (phi_right + phi_left);
    double angular = wheel_radius_ / wheel_separation_ * (phi_right - phi_left);
